    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-indigo-600 via-purple-500 to-blue-400 text-white">
      <div className="w-full max-w-md p-8 m-4 space-y-8 bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20">
        ...
        <div className="text-center">
          <h1 className="text-3xl font-extrabold tracking-tight text-white drop-shadow">{t("registerTitle")}</h1>
          <p className="mt-2 text-sm text-white/80">
            {t("haveAccount")}{" "}
            <Link href={`/${data.locale}/auth/login`} className="font-semibold text-yellow-300 hover:underline transition">
              {t("signIn")}
            </Link>
          </p>
        </div>
            ...
            render={({ field }) => (
              <div>
                ...
                <input
                  ...
                  autoComplete="email"
                  placeholder={t("email")}
                  className="relative block w-full px-3 py-3 bg-white/80 border border-white/30 rounded-lg shadow-sm text-indigo-900 placeholder-indigo-400 focus:outline-none focus:ring-2 focus:ring-yellow-300 focus:border-yellow-300 sm:text-sm transition"
                />
